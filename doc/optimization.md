...
- 維持 Channel Points 與 Twitch Drops 的觀看進度。
...
不建議直接停止影片、攔截 HLS media、快速輪流切換單一 Page，因為這些作法可能讓 Channel Points 或 Drops 不再計入有效觀看。
...
- Channel Points 與 Drops 仍可累積。
- Bonus Points 與 Drops 領取功能保持正常。
...
- 比較 Page 首次載入傳輸量。
- 確認 player、Points、Drops、登入狀態與截圖正常。
...
- 每 30 秒執行一次健康檢查。
- 每 15 秒執行一次 Bonus Points 與 Drops 入口。
- Drops Claimer 內部另有 60 秒全域冷卻。
...
Bonus Points 通常不需要 15 秒內立即領取；30 秒仍有足夠反應速度。Drops 已有共用 60 秒冷卻，不需要每個 Page 同時頻繁進入檢查。
...
可能讓 Channel Points、Drops 或 Twitch 觀看心跳停止，不符合核心需求。
...
無法確認登入狀態、Channel Points 與 Drops 的有效觀看計算與目前完整 Twitch 頁一致，風險高。

### 隱藏、凍結或 Background Throttling Page

瀏覽器可能降低 timer 與 media 活性，並影響 Twitch 判定觀看狀態。除非經長時間實測確認 Points 與 Drops 仍增加，否則不採用。
...
7. 用 benchmark 與長時間 Points／Drops 實測決定是否採用為預設。
