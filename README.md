...
主要事件包含 `service_started`、`config_loaded`、`credential_checked`、`stream_online`、`stream_offline`、`watch_started`、`watch_stopped`、`reward_claimed`、`reward_claim_failed`、`browser_restarted`、`page_health_failed` 與 `service_stopped`。日誌會遮罩 cookie、token、Authorization header 與 storageState 內容。
...
- 不提供 Web UI、通知系統、健康檢查端點或自動 token 更新。
